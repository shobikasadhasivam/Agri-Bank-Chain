...
app.post('/channels/mychannel/chaincodes/agrichain', async (req, res) => {
  try {
    ...
    console.log('Received request:', JSON.stringify({ fcn, args }, null, 2));
    ...
    // MANUAL CHECK - Force ValidateHybridConsensus to use QUERY
    if (fcn === 'ValidateHybridConsensus') {
      command = `peer chaincode query -C mychannel -n agrichain -c '{"function":"${fcn}","Args":[${argsString}]}'`;
      console.log('🔍 FORCING QUERY for ValidateHybridConsensus');
    } else if (fcn.startsWith('Get')) {
      command = `peer chaincode query -C mychannel -n agrichain -c '{"function":"${fcn}","Args":[${argsString}]}'`;
      console.log('📖 Using QUERY for Get function');
    } else {
...
app.listen(3000, () => {
  ...
  console.log('This will FORCE ValidateHybridConsensus to use QUERY operation');
});
